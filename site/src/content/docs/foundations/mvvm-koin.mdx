...
> - **אין `KoinComponent` בקוד production** — העדיפו constructor injection (תבנית ה-DI של הצוות לפי §5 ב-`CLAUDE.md`: `viewModelOf(::Impl) bind Interface`). (תראו `KoinComponent` בכמה מקומות ישנים כמו `SearchViewModelImpl` — חריג שמנוגד לתבנית; אל תחקו אותו בקוד חדש.)
...
fun `Change text input fetch search results after debounce`() = runTest {
    searchVM.onInputTextChange("ss")                          // 1. fire an event

    assertIs<SearchMode.Basic.Loading>(searchVM.state.value.searchMode)  // 2. intermediate state

    advanceUntilIdle()                                        // 3. run the debounce

    assertEquals(                                             // 4. assert the final state
        desiredTitleList,
...
- וקראו **שוב** את **§5 Conventions** (ו-§2 Modules) ב-`CLAUDE.md` של alpha-mobile — עכשיו, כשכל מילה בו אומרת משהו.
