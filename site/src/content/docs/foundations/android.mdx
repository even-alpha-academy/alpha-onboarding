...
> **גשר:** flavors ו-`BuildConfig` ≈ קובצי `.env` שנפתרים בזמן קומפילציה. המקרה האמיתי אצלנו: המאפיין `alpha.connectivity` (ועוד ערכי `alpha.*`, מוזרקים מ-`local.properties` בצד הסגור) נכנס ל-`BuildConfig` — למשל `BuildConfig.IS_EXTERNAL` — ושולט אם הבילד מתחבר ל-mocks או לאמיתי.
