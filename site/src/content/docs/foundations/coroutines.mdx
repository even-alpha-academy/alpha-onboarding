...
<VideoLink
  url="https://www.youtube.com/watch?v=e7tKQDJsTGs"
  title="Coroutines: Concurrency in Kotlin"
  channel="Dave Leeds"
  duration="30 דק׳"
  why="הסבר מאויר ונקי של בדיוק המודל המנטלי שהמסמך הזה בונה — structured concurrency."
  segment="הפרקים Concurrency, Cancellation ו-Partial Cancellation הם הליבה (הסרטון לא נוגע ב-Flow — לזה יש את החלק השני של המסמך)."
/>
...
```kotlin
val user = loadUser(id)   // looks synchronous, doesn't block the thread
```
...
val a = async { loadA() }
val b = async { loadB() }
val combined = a.await() + b.await()   // A and B ran in parallel
```
...
```kotlin
val data = withContext(dispatchers.io) { readBigFile() }  // runs on IO, returns to caller
```
...
## תרגיל — קורוטינות וביטול

קובץ scratch של JVM מספיק. כתבו `suspend fun` ש"שולפת" שני משאבים מדומים במקביל (`async`) ומשלבת אותם. עטפו ב-`withTimeout(2000)` והדפיסו הוכחה שהביטול התפשט (למשל הדפסה בתוך `finally`, או תפיסת `CancellationException`).

> **הגדרת סיום:** התרגיל רץ, ואתם יכולים לענות "מי מבטל את הקורוטינה הזו, ומתי" על **כל** coroutine שהשקתם.
...
val state: StateFlow<ScreenState> = _state          // immutable to the outside
...
@Test
fun example() = runTest {
    vm.onInputTextChange("ss")        // launches debounced work
    advanceUntilIdle()                // "runs" all the pending virtual time
...
> **הגדרת סיום:** ה-Flow מתרנדר לתוך ה-`StateFlow`, ואתם יכולים להסביר במילים שלכם מה זה conflation ולמה ערכי ביניים נבלעו.
