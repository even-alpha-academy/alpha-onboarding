...
**ב. Flow:**
בנו `Flow<Int>` מסוג ticker, הפעילו עליו `map` + `filter`, ואספו אותו לתוך `MutableStateFlow`. אמתו את הערך האחרון. אחר כך הסבירו (למנטור, או בהערה) **למה `StateFlow` השמיט ערכי ביניים** — זהו רמז: conflation. זו נקודת ה-aha של המסמך.
