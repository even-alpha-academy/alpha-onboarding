...
> **מה זה "קוד אידיומטי"?** קוד **אידיומטי** הוא קוד שכתוב *ברוח השפה* — כמו שדובר שפת-אם היה כותב אותו, ולא תרגום מילולי משפה אחרת. ההפך הוא **"Java-ism"** (או "TS-ism"): קוד שמתקמפל ועובד, אבל "מדבר במבטא" של שפת המקור — למשל `if/else` ארוך במקום `when`, לולאת אינדקס במקום `map`/`filter`, או `class` רגיל במקום `data`/`sealed`. המסמך הזה מצביע לכל אורכו על מה אידיומטי ב-Kotlin ומה כדאי להחליף.
...
| `switch (x) { … }` | `when (x) { … -> … }` | אבל `when` הוא **ביטוי** (מחזיר ערך) ו**ממצה** — ראו [`sealed` ו-`when`](#מחלקות-ו-data) |
...
```kotlin
val name: String   = "Dana"   // never null
val maybe: String? = null      // nullable — the compiler forces you to handle it
```
...
maybe?.let { use(it) } // run the block only when non-null
maybe ?: "default"     // default value (Elvis)
maybe!!.length         // "trust me, not null" — throws NPE if you're wrong
```
...
```kotlin
val updated = area.copy(capacity = 30)  // a new object; the rest is unchanged
```
...
```kotlin
val text = when (mode) {        // no `else` needed once every branch is covered
    SearchMode.Initial -> "—"
...
"abc123".isValidId()   // reads as if it were a method on String
```
...
fetch(sort = false)   // named argument; limit stays 50
```
...
```kotlin
val lazyValue: String by lazy { computeExpensive() }  // computed once, on first access
class MyList(impl: List<Int>) : List<Int> by impl     // delegates the whole interface to impl
```
