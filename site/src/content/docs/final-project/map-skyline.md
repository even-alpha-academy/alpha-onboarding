...
## התיעוד הרשמי (ISGWorld)

יש תיעוד API רשמי ל-SDK, וכדאי שיהיה פתוח לידכם לכל אורך השלב: **[ISGWorld 8.0 — Programmer's Guide](https://www.skylinesoft.com/KB_Resources/TED/WebHelp/Programmers%20Guide/ISGWorld80.htm)**.

`ISGWorld` הוא נקודת הכניסה המרכזית ל-TerraExplorer — בדיוק האובייקט ש-`ISGWorld.getInstance()` מחזיר בקוד של alpha-mobile. המדריך מתעד את ה-properties, ה-methods וה-events (ניווט, פתיחת פרויקט, יצירת אובייקטים, terrain, מדידות, ועוד). הדוגמאות שם הן ב-JavaScript / C# / C++ — אתם מתרגמים את ה**מושגים** ל-Kotlin (ה-API זהה; רק התחביר שונה).

> **זה המקור הרשמי לאימות.** כל קריאת TE שאינכם בטוחים בה — בדקו אותה מול המדריך הזה ומול ה-wrappers של alpha-mobile, *לפני* שאתם מסתמכים עליה. (ראו את [שיעור ה-AI](#שיעור-ה-ai) למה זה קריטי דווקא כאן.)
...
> **ה-AI יודע מעט מאוד על ה-SDK הזה — מעט, אבל לא אפס.** התיעוד הציבורי דליל ונתוני האימון דלילים, ולכן ה-Generation **ימציא** APIs של TerraExplorer שנראים אמינים לחלוטין ואינם קיימים. ההבדל מ-Kotlin רגיל: שם טעות נדירה; כאן היא ברירת המחדל.
>
> מה שמציל אתכם זה שיש **שני מקורות אמת** לבדוק מולם — וזה בדיוק מה שהופך את העבודה כאן למקצועית במקום ניחוש:
> 1. **התיעוד הרשמי** — ה-[ISGWorld Programmer's Guide](#התיעוד-הרשמי-isgworld). כל קריאה שה-AI הציע — ודאו שהיא קיימת שם.
> 2. **ה-wrappers של alpha-mobile** — קוד אמיתי שעובד, מול ה-SDK הזה בדיוק.
>
> אז במקום "לסמוך על ה-AI" או "להתעלם מה-AI", העבודה היא: **בקשו מה-AI טיוטה/כיוון ← אמתו כל קריאה מול המדריך הרשמי ומול ה-wrappers ← כתבו בעצמכם.** השתמשו ב-**Explainer** מעל קוד ה-wrappers ("הסבר לי מה הקובץ הזה עושה"), וב-decompiler/autocomplete של ה-IDE על ה-AAR.
>
> **מצב Generator אסור לקוד wrapper בשלב הזה** — לא כעונש, אלא כי טיוטה לא-מאומתת כאן כמעט תמיד שגויה. אמתו בעצמכם: הקדישו 15 דקות (detour מאושר) לבקש מה-AI לכתוב את האתחול, ואז ספרו כמה מהמתודות שלו באמת מופיעות במדריך הרשמי. זה מחסן טוב יותר מכל הרצאה.
...
## נפילת חופף (גלוי גם לכם)

אם ההקמה נלחמת בכם יותר מיום, **החופף מוסר לכם את קטע האתחול העובד** ואתם ממשיכים ל-wrapper. לוח הזמנים חשוב יותר מהסבל.
...
- הקשה פותחת bottom sheet, ו-fly-to עובד.
